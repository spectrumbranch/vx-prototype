...
h=Object.prototype,e=f.forEach,n=f.slice,u=h.hasOwnProperty,q=h.toString,x=m.Popcorn,v=[],o=false,z={events:{hash:{},apis:{}}},I=function(){return m.requestAnimationFrame||m.webkitRequestAnimationFrame||m.mozRequestAnimationFrame||m.oRequestAnimationFrame||m.msRequestAnimationFrame||function(a){m.setTimeout(a,16)}}(),E={put:function(a){for(var c in a)if(a.hasOwnProperty(c))this[c]=a[c]}},g=function(a,c){return new g.p.init(a,c||null)};g.version="8a40e8b";g.isSupported=true;g.instances=[];g.p=g.prototype=
{init:function(a,c){var l,j=this;if(typeof a==="function")if(i.readyState==="complete")a(i,g);else{v.push(a);if(!o){o=true;var s=function(){i.removeEventListener("DOMContentLoaded",s,false);for(var D=0,r=v.length;D<r;D++)v[D].call(i,g);v=null};i.addEventListener("DOMContentLoaded",s,false)}}else{if(typeof a==="string")try{l=i.querySelector(a)}catch(t){throw Error("Popcorn.js Error: Invalid media element selector: "+a);}this.media=l||a;l=this.media.nodeName&&this.media.nodeName.toLowerCase()||"video";
...
r,L,w,p;D=j.media.duration;D=D!=D?Number.MAX_VALUE:D+1;g.addTrackEvent(j,{start:D,end:D});if(!j.isDestroyed){j.data.durationChange=function(){var B=j.media.duration,Q=B+1,K=j.data.trackEvents.byStart,M=j.data.trackEvents.byEnd;K.pop();M.pop();for(var C=M.length-1;C>0;C--)M[C].end>B&&j.removeTrackEvent(M[C]._id);for(M=0;M<K.length;M++)K[M].end>B&&j.removeTrackEvent(K[M]._id);j.data.trackEvents.byEnd.push({start:Q,end:Q});j.data.trackEvents.byStart.push({start:Q,end:Q})};j.media.addEventListener("durationchange",
j.data.durationChange,false)}if(j.options.frameAnimation){j.data.timeUpdate=function(){g.timeUpdate(j,{});g.forEach(g.manifest,function(B,Q){if(r=j.data.running[Q]){w=r.length;for(var K=0;K<w;K++){L=r[K];(p=L._natives)&&p.frame&&p.frame.call(j,{},L,j.currentTime())}}});j.emit("timeupdate");!j.isDestroyed&&I(j.data.timeUpdate)};!j.isDestroyed&&I(j.data.timeUpdate)}else{j.data.timeUpdate=function(B){g.timeUpdate(j,B)};j.isDestroyed||j.media.addEventListener("timeupdate",j.data.timeUpdate,false)}};j.media.addEventListener("error",
...
g.extend=function(a){var c=n.call(arguments,1);g.forEach(c,function(l){for(var j in l)a[j]=l[j]});return a};g.extend(g,{noConflict:function(a){if(a)m.Popcorn=x;return g},error:function(a){throw Error(a);},guid:function(a){g.guid.counter++;return(a?a:"")+(+new Date+g.guid.counter)},sizeOf:function(a){var c=0,l;for(l in a)c++;return c},isArray:Array.isArray||function(a){return q.call(a)==="[object Array]"},nop:function(){},position:function(a){if(!a.parentNode)return null;a=a.getBoundingClientRect();
...
g.Events.Natives=g.Events.UIEvents+" "+g.Events.MouseEvents+" "+g.Events.Events;z.events.apiTypes=["UIEvents","MouseEvents","Events"];(function(a,c){for(var l=z.events.apiTypes,j=a.Natives.split(/\s+/g),s=0,t=j.length;s<t;s++)c.hash[j[s]]=true;l.forEach(function(y){c.apis[y]={};for(var D=a[y].split(/\s+/g),r=D.length,L=0;L<r;L++)c.apis[y][D[L]]=true})})(g.Events,z.events);g.events={isNative:function(a){return!!z.events.hash[a]},getInterface:function(a){if(!g.events.isNative(a))return false;var c=
...
a.end<this.parent.data.trackEvents.previousUpdateTime&&this.parent.data.trackEvents.endIndex++;this.count++};d.prototype.remove=function(a){if(a instanceof k)a=a.id;if(typeof a==="object"){this.where(a).forEach(function(w){this.removeTrackEvent(w._id)},this.parent);return this}var c,l,j;j=this.byStart.length;for(var s=0,t=0,y=[],D=[],r=[],L=[];--j>-1;){c=this.byStart[s];l=this.byEnd[s];if(!c._id){y.push(c);D.push(l)}if(c._id){c._id!==a&&y.push(c);l._id!==a&&D.push(l);if(c._id===a)t=s}s++}j=this.animating.length;
s=0;if(j)for(;--j>-1;){c=this.animating[s];c._id||r.push(c);c._id&&c._id!==a&&r.push(c);s++}t<=this.startIndex&&this.startIndex--;t<=this.endIndex&&this.endIndex--;this.byStart=y;this.byEnd=D;this.animating=r;this.count--;j=this.parent.data.history.length;for(s=0;s<j;s++)this.parent.data.history[s]!==a&&L.push(this.parent.data.history[s]);this.parent.data.history=L};g.addTrackEvent=function(a,c){var l;if(!(c instanceof k)){if((c=new k(c))&&c._natives&&c._natives.type&&a.options.defaults&&a.options.defaults[c._natives.type]){l=
...
function(a){return a.data.trackRefs};g.getTrackEvent=function(a,c){return a.data.trackRefs[c]};g.getTrackEvent.ref=function(a,c){return a.data.trackRefs[c]};g.getLastTrackEventId=function(a){return a.data.history[a.data.history.length-1]};g.timeUpdate=function(a,c){var l=a.media.currentTime,j=a.data.trackEvents.previousUpdateTime,s=a.data.trackEvents,t=s.endIndex,y=s.startIndex,D=s.byStart.length,r=s.byEnd.length,L=g.registryByName,w,p,B;if(j<=l){for(;s.byEnd[t]&&s.byEnd[t].end<=l;){w=s.byEnd[t];
p=(j=w._natives)&&j.type;if(!j||L[p]||a[p]){if(w._running===true){w._running=false;B=a.data.running[p];B.splice(B.indexOf(w),1);if(!a.data.disabled[p]){j.end.call(a,c,w);a.emit("trackend",g.extend({},w,{plugin:p,type:"trackend",track:w}))}}t++}else{g.removeTrackEvent(a,w._id);return}}for(;s.byStart[y]&&s.byStart[y].start<=l;){w=s.byStart[y];p=(j=w._natives)&&j.type;if(!j||L[p]||a[p]){if(w.end>l&&w._running===false){w._running=true;a.data.running[p].push(w);if(!a.data.disabled[p]){j.start.call(a,c,
w);a.emit("trackstart",g.extend({},w,{plugin:p,type:"trackstart",track:w}))}}y++}else{g.removeTrackEvent(a,w._id);return}}}else if(j>l){for(;s.byStart[y]&&s.byStart[y].start>l;){w=s.byStart[y];p=(j=w._natives)&&j.type;if(!j||L[p]||a[p]){if(w._running===true){w._running=false;B=a.data.running[p];B.splice(B.indexOf(w),1);if(!a.data.disabled[p]){j.end.call(a,c,w);a.emit("trackend",g.extend({},w,{plugin:p,type:"trackend",track:w}))}}y--}else{g.removeTrackEvent(a,w._id);return}}for(;s.byEnd[t]&&s.byEnd[t].end>
l;){w=s.byEnd[t];p=(j=w._natives)&&j.type;if(!j||L[p]||a[p]){if(w.start<=l&&w._running===false){w._running=true;a.data.running[p].push(w);if(!a.data.disabled[p]){j.start.call(a,c,w);a.emit("trackstart",g.extend({},w,{plugin:p,type:"trackstart",track:w}))}}t--}else{g.removeTrackEvent(a,w._id);return}}}s.endIndex=t;s.startIndex=y;s.previousUpdateTime=l;s.byStart.length<D&&s.startIndex--;s.byEnd.length<r&&s.endIndex--};g.extend(g.p,{getTrackEvents:function(){return g.getTrackEvents.call(null,this)},
...
0)g.error("'"+a+"' is a protected function name");else{var j=typeof c==="function",s=["start","end","type","manifest"],t=["_setup","_teardown","start","end","frame"],y={},D=function(w,p){w=w||g.nop;p=p||g.nop;return function(){w.apply(this,arguments);p.apply(this,arguments)}};g.manifest[a]=l=l||c.manifest||{};t.forEach(function(w){c[w]=b(c[w]||g.nop,a)});var r=function(w,p){if(!p)return this;if(p.ranges&&g.isArray(p.ranges)){g.forEach(p.ranges,function(M){M=g.extend({},p,M);delete M.ranges;this[a](M)},
this);return this}var B=p._natives={},Q="",K;g.extend(B,w);p._natives.type=p._natives.plugin=a;p._running=false;B.start=B.start||B["in"];B.end=B.end||B.out;if(p.once)B.end=D(B.end,function(){this.removeTrackEvent(p._id)});B._teardown=D(function(){var M=n.call(arguments),C=this.data.running[B.type];M.unshift(null);M[1]._running&&C.splice(C.indexOf(p),1)&&B.end.apply(this,M);M[1]._running=false;this.emit("trackend",g.extend({},p,{plugin:B.type,type:"trackend",track:g.getTrackEvent(this,p.id||p._id)}))},
B._teardown);B._teardown=D(B._teardown,function(){this.emit("trackteardown",g.extend({},p,{plugin:a,type:"trackteardown",track:g.getTrackEvent(this,p.id||p._id)}))});p.compose=p.compose||[];if(typeof p.compose==="string")p.compose=p.compose.split(" ");p.effect=p.effect||[];if(typeof p.effect==="string")p.effect=p.effect.split(" ");p.compose=p.compose.concat(p.effect);p.compose.forEach(function(M){Q=g.compositions[M]||{};t.forEach(function(C){B[C]=D(B[C],Q[C])})});p._natives.manifest=l;if(!("start"in
p))p.start=p["in"]||0;if(!p.end&&p.end!==0)p.end=p.out||Number.MAX_VALUE;if(!u.call(p,"toString"))p.toString=function(){var M=["start: "+p.start,"end: "+p.end,"id: "+(p.id||p._id)];p.target!=null&&M.push("target: "+p.target);return a+" ( "+M.join(", ")+" )"};if(!p.target){K="options"in l&&l.options;p.target=K&&"target"in K&&K.target}if(!p._id&&p._natives)p._id=g.guid(p._natives.type);if(p instanceof k){if(p._natives){p._id=p.id||p._id||g.guid(p._natives.type);if(p._natives._setup){p._natives._setup.call(this,
p);this.emit("tracksetup",g.extend({},p,{plugin:p._natives.type,type:"tracksetup",track:p}))}}this.data.trackEvents.add(p);k.start(this,p);this.timeUpdate(this,null,true);p._id&&g.addTrackEvent.ref(this,p)}else g.addTrackEvent(this,p);g.forEach(w,function(M,C){s.indexOf(C)===-1&&this.on(C,M)},this);return this};g.p[a]=y[a]=function(w,p){var B,Q;if(w&&!p)p=w;else if(B=this.getTrackEvent(w)){Q=p;var K={},M;for(M in B)if(u.call(Q,M)&&u.call(B,M))K[M]=B[M];if(B._natives._update){this.data.trackEvents.remove(B);
if(u.call(p,"start"))B.start=p.start;if(u.call(p,"end"))B.end=p.end;k.end(this,B);j&&c.call(this,B);B._natives._update.call(this,B,p);this.data.trackEvents.add(B);k.start(this,B)}else{g.extend(B,p);this.data.trackEvents.remove(w);B._natives._teardown&&B._natives._teardown.call(this,B);g.removeTrackEvent.ref(this,w);if(j)r.call(this,c.call(this,B),B);else{B._id=B.id||B._id||g.guid(B._natives.type);if(B._natives&&B._natives._setup){B._natives._setup.call(this,B);this.emit("tracksetup",g.extend({},B,
{plugin:B._natives.type,type:"tracksetup",track:B}))}this.data.trackEvents.add(B);k.start(this,B);this.timeUpdate(this,null,true);g.addTrackEvent.ref(this,B)}this.emit("trackchange",{id:B.id,type:"trackchange",previousValue:K,currentValue:B,track:B});return this}B._natives.type!=="cue"&&this.emit("trackchange",{id:B.id,type:"trackchange",previousValue:K,currentValue:Q,track:B});return this}else p.id=w;this.data.running[a]=this.data.running[a]||[];B=g.extend({},this.options.defaults&&this.options.defaults[a]||
{},p);r.call(this,j?c.call(this,B):c,B);return this};l&&g.extend(c,{manifest:l});var L={fn:y[a],definition:c,base:c,parents:[],name:a};g.registry.push(g.extend(y,L,{type:a}));g.registryByName[a]=L;return y}};g.plugin.errors=[];g.plugin.debug=g.version==="8a40e8b";g.removePlugin=function(a,c){if(!c){c=a;a=g.p;if(g.protect.natives.indexOf(c.toLowerCase())>=0){g.error("'"+c+"' is a protected function name");return}var l=g.registry.length,j;for(j=0;j<l;j++)if(g.registry[j].name===c){g.registry.splice(j,
...
c)}};return c};g.xhr.getJSONP=function(a,c,l){var j=i.head||i.getElementsByTagName("head")[0]||i.documentElement,s=i.createElement("script"),t=false,y=[];y=/(=)\?(?=&|$)|\?\?/;var D,r;if(!l){r=a.match(/(callback=[^&]*)/);if(r!==null&&r.length){y=r[1].split("=")[1];if(y==="?")y="jsonp";D=g.guid(y);a=a.replace(/(callback=[^&]*)/,"callback="+D)}else{D=g.guid("jsonp");if(y.test(a))a=a.replace(y,"$1"+D);y=a.split(/\?(.+)?/);a=y[0]+"?";if(y[1])a+=y[1]+"&";a+="callback="+D}window[D]=function(L){c&&c(L);
t=true}}s.addEventListener("load",function(){l&&c&&c();t&&delete window[D];j.removeChild(s)},false);s.addEventListener("error",function(L){c&&c({error:L});l||delete window[D];j.removeChild(s)},false);s.src=a;j.insertBefore(s,j.firstChild)};g.getJSONP=g.xhr.getJSONP;g.getScript=g.xhr.getScript=function(a,c){return g.xhr.getJSONP(a,c,true)};g.util={toSeconds:function(a,c){var l=/^([0-9]+:){0,2}[0-9]+([.;][0-9]+)?$/,j,s,t;if(typeof a==="number")return a;typeof a==="string"&&!l.test(a)&&g.error("Invalid time format");
...
function(){}}})(window,window.document);(function(m,i){function k(f){f=typeof f==="string"?f:[f.language,f.region].join("-");var h=f.split("-");return{iso6391:f,language:h[0]||"",region:h[1]||""}}var d=m.navigator,b=k(d.userLanguage||d.language);i.locale={get:function(){return b},set:function(f){b=k(f);i.locale.broadcast();return b},broadcast:function(f){var h=i.instances,e=h.length,n=0,u;for(f=f||"locale:changed";n<e;n++){u=h[n];f in u.data.events&&u.trigger(f)}}}})(this,this.Popcorn);(function(m){var i=Object.prototype.hasOwnProperty;m.parsers={};m.parser=function(k,d,b){if(m.protect.natives.indexOf(k.toLowerCase())>=0)m.error("'"+k+"' is a protected function name");else{if(typeof d==="function"&&!b){b=d;d=""}if(!(typeof b!=="function"||typeof d!=="string")){var f={};f[k]=function(h,e,n){if(!h)return this;if(typeof e!=="function"&&!n){n=e;e=null}var u=this;m.xhr({url:h,dataType:d,success:function(q){var x,v,o=0;q=b(q,n).data||[];if(x=q.length){for(;o<x;o++){v=q[o];for(var z in v)i.call(v,
z)&&u[z]&&u[z](v[z])}e&&e()}}});return this};m.extend(m.p,f);return f}}}})(Popcorn);(function(m){var i=function(k,d){k=k||m.nop;d=d||m.nop;return function(){k.apply(this,arguments);d.apply(this,arguments)}};m.player=function(k,d){if(!m[k]){d=d||{};var b=function(f,h,e){e=e||{};var n=new Date/1E3,u=n,q=0,x=0,v=1,o=false,z={},I=typeof f==="string"?m.dom.find(f):f,E={};Object.prototype.__defineGetter__||(E=I||document.createElement("div"));for(var g in I)if(!(g in E))if(typeof I[g]==="object")E[g]=I[g];else if(typeof I[g]==="function")E[g]=function(A){return"length"in I[A]&&!I[A].call?
I[A]:function(){return I[A].apply(I,arguments)}}(g);else m.player.defineProperty(E,g,{get:function(A){return function(){return I[A]}}(g),set:m.nop,configurable:true});var F=function(){n=new Date/1E3;if(!E.paused){E.currentTime+=n-u;E.dispatchEvent("timeupdate");setTimeout(F,10)}u=n};E.play=function(){this.paused=false;if(E.readyState>=4){u=new Date/1E3;E.dispatchEvent("play");F()}};E.pause=function(){this.paused=true;E.dispatchEvent("pause")};m.player.defineProperty(E,"currentTime",{get:function(){return q},
set:function(A){q=+A;E.dispatchEvent("timeupdate");return q},configurable:true});m.player.defineProperty(E,"volume",{get:function(){return v},set:function(A){v=+A;E.dispatchEvent("volumechange");return v},configurable:true});m.player.defineProperty(E,"muted",{get:function(){return o},set:function(A){o=+A;E.dispatchEvent("volumechange");return o},configurable:true});m.player.defineProperty(E,"readyState",{get:function(){return x},set:function(A){return x=A},configurable:true});E.addEventListener=function(A,
O){z[A]||(z[A]=[]);z[A].push(O);return O};E.removeEventListener=function(A,O){var a,c=z[A];if(c){for(a=z[A].length-1;a>=0;a--)O===c[a]&&c.splice(a,1);return O}};E.dispatchEvent=function(A){var O,a=A.type;if(!a){a=A;if(A=m.events.getInterface(a)){O=document.createEvent(A);O.initEvent(a,true,true,window,1)}}if(z[a])for(A=z[a].length-1;A>=0;A--)z[a][A].call(this,O,this)};E.src=h||"";E.duration=0;E.paused=true;E.ended=0;e&&e.events&&m.forEach(e.events,function(A,O){E.addEventListener(O,A,false)});if(d._canPlayType(I.nodeName,
...
b.get||m.nop);k.__defineSetter__(d,b.set||m.nop)};m.player.playerQueue=function(){var k=[],d=false;return{next:function(){d=false;k.shift();k[0]&&k[0]()},add:function(b){k.push(function(){d=true;b&&b()});!d&&k[0]()}}};m.smart=function(k,d,b){var f=typeof k==="string"?m.dom.find(k):k,h,e,n,u,q,x="HTMLYouTubeVideoElement HTMLVimeoVideoElement HTMLSoundCloudAudioElement HTMLNullVideoElement".split(" ");if(f){d=typeof d==="string"?[d]:d;k=0;for(q=d.length;k<q;k++){h=d[k];for(e=0;e<x.length;e++)if((u=
m[x[e]])&&u._canPlaySrc(h)==="probably"){n=u(f);b=m(n,b);setTimeout(function(){n.src=h},0);return b}for(var v in m.player.registry)if(m.player.registry.hasOwnProperty(v))if(m.player.registry[v].canPlayType(f.nodeName,h))return m[v](f,h,b)}var o;v=m.guid("popcorn-video-");e=document.createElement("div");e.style.width="100%";e.style.height="100%";if(d.length===1){o=document.createElement("video");o.id=v;f.appendChild(o);setTimeout(function(){var z=document.createElement("div");z.innerHTML=d[0];o.src=
z.firstChild.nodeValue},0);return m("#"+v,b)}f.appendChild(e);x='<video id="'+v+'" preload=auto autobuffer>';k=0;for(q=d.length;k<q;k++)x+='<source src="'+d[k]+'">';x+="</video>";e.innerHTML=x;b&&b.events&&b.events.error&&f.addEventListener("error",b.events.error,false);return m("#"+v,b)}else m.error("Specified target `"+k+"` was not found.")}})(Popcorn);(function(m){document.addEventListener("DOMContentLoaded",function(){var i=document.querySelectorAll("[data-timeline-sources]");m.forEach(i,function(k,d){var b=i[d],f,h,e;if(!b.id)b.id=m.guid("__popcorn");if(b.nodeType&&b.nodeType===1){e=m("#"+b.id);f=(b.getAttribute("data-timeline-sources")||"").split(",");f[0]&&m.forEach(f,function(n){h=n.split("!");if(h.length===1){h=n.match(/(.*)[\/\\]([^\/\\]+\.\w+)$/)[2].split(".");h[0]="parse"+h[1].toUpperCase();h[1]=n}f[0]&&e[h[0]]&&e[h[0]](h[1])});e.autoplay()&&
...
defaultPlaybackRate:{get:function(){return 1},configurable:true},style:{get:function(){return this.parentNode.style},configurable:true},id:{get:function(){return this.parentNode.id},configurable:true}});return b}})(Popcorn,window.document);(function(m,i){function k(){return"maybe"}function d(b,f){var h=typeof b==="string"?i.querySelector(b):b,e=i.createElement(f);h.appendChild(e);e._canPlaySrc=k;return e}m.HTMLVideoElement=function(b){return d(b,"video")};m.HTMLVideoElement._canPlaySrc=k;m.HTMLAudioElement=function(b){return d(b,"audio")};m.HTMLAudioElement._canPlaySrc=k})(Popcorn,window.document);(function(m,i,k){function d(){if(i.jwplayer){n=true;for(var x=q.length;x--;){q[x]();delete q[x]}}else setTimeout(d,100)}function b(){if(!u){if(!i.jwplayer){var x=k.createElement("script");x.src="https://jwpsrv.com/library/zaIF4JI9EeK2FSIACpYGxA.js";var v=k.getElementsByTagName("script")[0];v.parentNode.insertBefore(x,v)}u=true;d()}return n}function f(x){q.unshift(x)}function h(x){function v(P){C.unshift(P)}function o(){setTimeout(function(){r.duration=K.getDuration();y.dispatchEvent("durationchange");
r.readyState=y.HAVE_METADATA;y.dispatchEvent("loadedmetadata");y.dispatchEvent("loadeddata");r.readyState=y.HAVE_FUTURE_DATA;y.dispatchEvent("canplay");for(B=true;C.length;){C[0]();C.shift()}r.readyState=y.HAVE_ENOUGH_DATA;y.dispatchEvent("canplaythrough")},0)}function z(){if(w)w=false;else if(H){H=false;o()}else l()}function I(){if(r.seeking){r.ended=false;r.seeking=false;y.dispatchEvent("timeupdate");y.dispatchEvent("seeked");y.dispatchEvent("canplay");y.dispatchEvent("canplaythrough")}}function E(){K.onPause(z);
K.onTime(function(){if(!r.ended&&!r.seeking){r.currentTime=K.getPosition();y.dispatchEvent("timeupdate")}});K.onSeek(I);K.onPlay(function(){if(!r.ended)if(T){T=false;if(r.autoplay||!r.paused){r.paused=false;v(a);o()}else{p=H=true;K.pause(true)}}else if(p){p=false;w=true;K.pause(true)}else a()});K.onBufferChange(c);K.onComplete(j);K.play(true)}function g(P){var S={name:"MediaError"};S.message=P.message;S.code=P.code||5;r.error=S;y.dispatchEvent("error")}function F(P){if(y._canPlaySrc(P)){var S=y._util.parseUri(P).queryKey;
r.controls=S.controls=S.controls||r.controls;r.src=P;if(b()){if(L)L&&K&&K.destroy();jwplayer(D.id).setup({file:P,width:"100%",height:"100%",controls:r.controls});K=jwplayer(D.id);K.onReady(E);K.onError(g);jwplayer.utils.log=function(G,V){if(typeof console!=="undefined"&&typeof console.log!=="undefined")V?console.log(G,V):console.log(G);G==="No suitable players found and fallback enabled"&&g({message:G,code:4})};r.networkState=y.NETWORK_LOADING;y.dispatchEvent("loadstart");y.dispatchEvent("progress")}else f(function(){F(P)})}else{r.error=
{name:"MediaError",message:"Media Source Not Supported",code:MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED};y.dispatchEvent("error")}}function A(P){r.currentTime=P;if(B){O();K.seek(P)}else v(function(){O();K.seek(P)})}function O(){r.seeking=true;if(r.paused)p=true;y.dispatchEvent("seeking")}function a(){r.paused=false;if(M){M=false;if(r.loop&&!Q||!r.loop){Q=true;y.dispatchEvent("play")}y.dispatchEvent("playing")}}function c(){y.dispatchEvent("progress")}function l(){r.paused=true;if(!M){M=true;y.dispatchEvent("pause")}}
function j(){if(r.loop)A(0);else{r.ended=true;l();y.dispatchEvent("timeupdate");y.dispatchEvent("ended")}}function s(P){r.volume=P;if(B){K.setVolume(r.volume*100);y.dispatchEvent("volumechange")}else v(function(){s(r.volume)})}function t(P){r.muted=P;if(B){K.setMute(P);y.dispatchEvent("volumechange")}else v(function(){t(r.muted)})}if(!i.postMessage)throw"ERROR: HTMLJWPlayerVideoElement requires window.postMessage";var y=new m._MediaElementProto,D=typeof x==="string"?k.querySelector(x):x,r={src:e,
networkState:y.NETWORK_EMPTY,readyState:y.HAVE_NOTHING,seeking:false,autoplay:e,preload:e,controls:false,loop:false,poster:e,volume:1,muted:false,currentTime:0,duration:NaN,ended:false,paused:true,error:null},L=false,w=false,p=false,B=false,Q=false,K,M=true,C=[],T=true,H=false;y._eventNamespace=m.guid("HTMLJWPlayerVideoElement::");y.parentNode=D;y._util.type="JWPlayer";y.play=function(){y.dispatchEvent("play");r.paused=false;if(B){if(r.ended){A(0);r.ended=false}K.play(true)}else v(function(){y.play()})};
y.pause=function(){r.paused=true;B?K.pause(true):v(function(){y.pause()})};Object.defineProperties(y,{src:{get:function(){return r.src},set:function(P){P&&P!==r.src&&F(P)}},autoplay:{get:function(){return r.autoplay},set:function(P){r.autoplay=y._util.isAttributeSet(P)}},loop:{get:function(){return r.loop},set:function(P){r.loop=y._util.isAttributeSet(P)}},width:{get:function(){return y.parentNode.offsetWidth}},height:{get:function(){return y.parentNode.offsetHeight}},currentTime:{get:function(){return r.currentTime},
set:function(P){A(P)}},duration:{get:function(){return K.getDuration()}},ended:{get:function(){return r.ended}},paused:{get:function(){return r.paused}},seeking:{get:function(){return r.seeking}},readyState:{get:function(){return r.readyState}},networkState:{get:function(){return r.networkState}},volume:{get:function(){return r.volume},set:function(P){if(P<0||P>1)throw"Volume value must be between 0.0 and 1.0";s(P)}},muted:{get:function(){return r.muted},set:function(P){t(y._util.isAttributeSet(P))}},
error:{get:function(){return r.error}},buffered:{get:function(){var P={start:function(S){if(S===0)return 0;throw"INDEX_SIZE_ERR: DOM Exception 1";},end:function(S){if(S===0){S=K.getDuration();if(!S)return 0;return S*(K.getBuffer()/100)}throw"INDEX_SIZE_ERR: DOM Exception 1";}};Object.defineProperties(P,{length:{get:function(){return 1}}});return P}}});y._canPlaySrc=m.HTMLJWPlayerVideoElement._canPlaySrc;y.canPlayType=m.HTMLJWPlayerVideoElement.canPlayType;return y}var e="",n=false,u=false,q=[];m.HTMLJWPlayerVideoElement=
function(x){return new h(x)};m.HTMLJWPlayerVideoElement._canPlaySrc=function(){return"probably"};m.HTMLJWPlayerVideoElement.canPlayType=function(){return"probably"}})(Popcorn,window,document);(function(m,i){function k(h){this.startTime=0;this.currentTime=h.currentTime||0;this.duration=h.duration||NaN;this.playInterval=null;this.paused=true;this.playbackRate=this.defaultPlaybackRate=1;this.ended=h.endedCallback||m.nop}function d(h){function e(a){A.push(a)}function n(){if(!E)return 0;return g.currentTime}function u(a){if(a!==n())if(E){F.seeking=true;o.dispatchEvent("seeking");g.seekTo(a);F.ended=false;F.seeking=false;o.dispatchEvent("timeupdate");o.dispatchEvent("seeked");o.dispatchEvent("canplay");
o.dispatchEvent("canplaythrough")}else e(function(){u(a)})}function q(){o.dispatchEvent("timeupdate")}function x(){F.paused=true;clearInterval(O);o.dispatchEvent("pause")}function v(){if(F.loop){u(0);o.play()}else{F.ended=true;x();o.dispatchEvent("timeupdate");o.dispatchEvent("ended")}}var o=new m._MediaElementProto,z=typeof h==="string"?i.querySelector(h):h,I=i.createElement("div"),E=false,g,F={src:b,networkState:o.NETWORK_EMPTY,readyState:o.HAVE_NOTHING,autoplay:b,preload:b,controls:b,loop:false,
poster:b,volume:1,muted:false,width:z.width|0?z.width:o._util.MIN_WIDTH,height:z.height|0?z.height:o._util.MIN_HEIGHT,seeking:false,ended:false,paused:1,error:null},A=[],O;o._eventNamespace=m.guid("HTMLNullVideoElement::");o.parentNode=z;o._util.type="NullVideo";o.play=function(){if(E){g.play();if(F.paused){if(F.paused===1){F.paused=false;o.dispatchEvent("play");o.dispatchEvent("playing")}else{if(F.ended){u(0);F.ended=false}if(F.paused){F.paused=false;F.loop||o.dispatchEvent("play");o.dispatchEvent("playing")}}O=
setInterval(q,o._util.TIMEUPDATE_MS)}}else e(function(){o.play()})};o.pause=function(){if(E){g.pause();F.paused||x()}else e(function(){o.pause()})};Object.defineProperties(o,{src:{get:function(){return F.src},set:function(a){if(a&&a!==F.src)if(o._canPlaySrc(a)){F.src=a;if(E)if(E&&g){g.pause();g=null;z.removeChild(I);I=i.createElement("div")}I.width=F.width;I.height=F.height;z.appendChild(I);a=f.exec(a);g=new k({currentTime:+a[1],duration:+a[2],endedCallback:v});o.dispatchEvent("loadstart");o.dispatchEvent("progress");
o.dispatchEvent("durationchange");E=true;F.networkState=o.NETWORK_IDLE;F.readyState=o.HAVE_METADATA;o.dispatchEvent("loadedmetadata");o.dispatchEvent("loadeddata");F.readyState=o.HAVE_FUTURE_DATA;o.dispatchEvent("canplay");F.readyState=o.HAVE_ENOUGH_DATA;for(o.dispatchEvent("canplaythrough");A.length;){a=A.shift();a()}F.autoplay&&o.play()}else{F.error={name:"MediaError",message:"Media Source Not Supported",code:MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED};o.dispatchEvent("error")}}},autoplay:{get:function(){return F.autoplay},
set:function(a){F.autoplay=o._util.isAttributeSet(a)}},loop:{get:function(){return F.loop},set:function(a){F.loop=o._util.isAttributeSet(a)}},width:{get:function(){return I.width},set:function(a){I.width=a;F.width=I.width}},height:{get:function(){return I.height},set:function(a){I.height=a;F.height=I.height}},currentTime:{get:function(){return n()},set:function(a){u(a)}},duration:{get:function(){return g?g.duration:NaN}},ended:{get:function(){return F.ended}},paused:{get:function(){return F.paused}},
seeking:{get:function(){return F.seeking}},readyState:{get:function(){return F.readyState}},networkState:{get:function(){return F.networkState}},volume:{get:function(){return F.volume},set:function(a){if(a<0||a>1)throw"Volume value must be between 0.0 and 1.0";F.volume=a;o.dispatchEvent("volumechange")}},muted:{get:function(){return F.muted},set:function(a){a=o._util.isAttributeSet(a);F.muted=a;o.dispatchEvent("volumechange")}},playbackRate:{get:function(){return g.playbackRate},set:function(a){g.playbackRate=
a;o.dispatchEvent("ratechange")}},error:{get:function(){return F.error}}});o._canPlaySrc=m.HTMLNullVideoElement._canPlaySrc;o.canPlayType=m.HTMLNullVideoElement.canPlayType;return o}var b="",f=/#t=(\d+\.?\d*)?,?(\d+\.?\d*)/;k.prototype={play:function(){var h=this;if(this.paused){this.paused=false;this.startTime=Date.now();this.playInterval=setInterval(function(){h.currentTime+=(Date.now()-h.startTime)/(1E3/h.playbackRate);h.startTime=Date.now();if(h.currentTime>=h.duration){h.pause(h.duration);h.ended()}h.currentTime<
0&&h.pause(0)},16)}},pause:function(){if(!this.paused){this.paused=true;clearInterval(this.playInterval)}},seekTo:function(h){h=h<0?0:h;this.currentTime=h=h>this.duration?this.duration:h}};m.HTMLNullVideoElement=function(h){return new d(h)};m.HTMLNullVideoElement._canPlaySrc=function(h){return f.test(h)?"probably":b};m.HTMLNullVideoElement.canPlayType=function(h){return h==="video/x-nullvideo"?"probably":b}})(Popcorn,document);(function(m,i,k){function d(){if(!u){m.getScript("https://w.soundcloud.com/player/api.js",function(){m.getScript("https://connect.soundcloud.com/sdk.js",function(){n=true;SC.initialize({client_id:"PRaNFlda6Bhf5utPjUsptg"});for(var x=q.length;x--;){q[x]();delete q[x]}})});u=true}return n}function b(x){q.unshift(x)}function f(x){function v(C){B.unshift(C)}function o(){p.bind(SC.Widget.Events.LOAD_PROGRESS,function(C){O({type:"loadProgress",data:C.currentPosition/1E3})});p.bind(SC.Widget.Events.PLAY_PROGRESS,
...
data:C})})});p.bind(SC.Widget.Events.FINISH,function(){O({type:"finish"})});L=true;p.getDuration(I)}function z(){p.bind(SC.Widget.Events.PLAY_PROGRESS,function(C){p.setVolume(0);if(C.currentPosition>0){p.unbind(SC.Widget.Events.PLAY_PROGRESS);p.bind(SC.Widget.Events.PAUSE,function(){p.unbind(SC.Widget.Events.PAUSE);p.setVolume(100);p.bind(SC.Widget.Events.SEEK,function(){p.unbind(SC.Widget.Events.SEEK);o()});p.seekTo(0)});p.pause()}});p.play()}function I(C){C/=1E3;var T=r.duration;if(T!==C){r.duration=
C;t.dispatchEvent("durationchange");if(isNaN(T)){r.networkState=t.NETWORK_IDLE;r.readyState=t.HAVE_METADATA;t.dispatchEvent("loadedmetadata");t.dispatchEvent("loadeddata");r.readyState=t.HAVE_FUTURE_DATA;t.dispatchEvent("canplay");r.readyState=t.HAVE_ENOUGH_DATA;t.dispatchEvent("canplaythrough");for(C=B.length;C--;){B[C]();delete B[C]}r.paused&&r.autoplay&&t.play()}}}function E(C){function T(){r.seeking=true;t.dispatchEvent("seeking");p.seekTo(C)}r.currentTime=C;C*=1E3;L?T():addMediaReadyCallback(T)}
function g(){r.paused=true;if(!w){w=true;clearInterval(Q);t.dispatchEvent("pause")}}function F(){t.dispatchEvent("timeupdate")}function A(C){r.currentTime=C;C!==M&&t.dispatchEvent("timeupdate");M=C}function O(C){switch(C.type){case "loadProgress":t.dispatchEvent("progress");break;case "playProgress":A(C.data);break;case "play":if(!K){K=setInterval(a,h);r.loop&&t.dispatchEvent("play")}Q=setInterval(F,t._util.TIMEUPDATE_MS);r.paused=false;if(w){w=false;r.loop||t.dispatchEvent("play");t.dispatchEvent("playing")}break;
case "pause":g();break;case "finish":if(r.loop){E(0);t.play()}else{r.ended=true;t.pause();g();t.dispatchEvent("timeupdate");t.dispatchEvent("ended")}break;case "seek":A(C.data)}}function a(){r.ended||p.getPosition(function(C){A(C/1E3)})}function c(C){if(t._canPlaySrc(C)){r.src=C;if(L)if(L&&p){clearInterval(K);p.pause();p.unbind(SC.Widget.Events.READY);p.unbind(SC.Widget.Events.LOAD_PROGRESS);p.unbind(SC.Widget.Events.PLAY_PROGRESS);p.unbind(SC.Widget.Events.PLAY);p.unbind(SC.Widget.Events.PAUSE);
p.unbind(SC.Widget.Events.SEEK);p.unbind(SC.Widget.Events.FINISH);y.removeChild(D);D=k.createElement("iframe")}if(d()){L=false;SC.get("/resolve",{url:C},function(T){var H;if(T.errors){H={name:"MediaError"};if(T.errors[0])if(T.errors[0].error_message==="404 - Not Found"){H.message="Video not found.";H.code=MediaError.MEDIA_ERR_NETWORK}r.error=H;t.dispatchEvent("error")}D.id=m.guid("soundcloud-");D.width=r.width;D.height=r.height;D.frameBorder=0;D.webkitAllowFullScreen=true;D.mozAllowFullScreen=true;
D.allowFullScreen=true;s(r.controls);y.appendChild(D);D.onload=function(){D.onload=null;p=SC.Widget(D);p.bind(SC.Widget.Events.READY,z);r.networkState=t.NETWORK_LOADING;t.dispatchEvent("loadstart");t.dispatchEvent("progress")};D.src="https://w.soundcloud.com/player/?url="+T.uri+"&show_artwork=false&buying=false&liking=false&sharing=false&download=false&show_comments=false&show_user=false&single_active=false"})}else b(function(){c(C)})}else{r.error={name:"MediaError",message:"Media Source Not Supported",
code:MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED};t.dispatchEvent("error")}}function l(C){r.volume=C;if(L){p.setVolume(C);t.dispatchEvent("volumechange")}else v(function(){l(C)})}function j(C){if(L)if(C){r.muted=r.volume;l(0)}else{r.muted=0;l(r.muted)}else{r.muted=C?1:0;v(function(){j(C)})}}function s(C){if(L){D.style.position="absolute";D.style.visibility=C?"visible":"hidden"}else{D.style.opacity=C?"1":"0";D.style.pointerEvents=C?"auto":"none"}r.controls=C}if(!i.postMessage)throw"ERROR: HTMLSoundCloudAudioElement requires window.postMessage";
var t=new m._MediaElementProto,y=typeof x==="string"?m.dom.find(x):x,D=k.createElement("iframe"),r={src:e,networkState:t.NETWORK_EMPTY,readyState:t.HAVE_NOTHING,seeking:false,autoplay:e,preload:e,controls:false,loop:false,poster:e,volume:100,muted:0,currentTime:0,duration:NaN,ended:false,paused:true,width:y.width|0?y.width:t._util.MIN_WIDTH,height:y.height|0?y.height:t._util.MIN_HEIGHT,error:null},L=false,w=true,p,B=[],Q,K,M=0;t._eventNamespace=m.guid("HTMLSoundCloudAudioElement::");t.parentNode=
y;t._util.type="SoundCloud";t.play=function(){r.paused=false;if(L){r.ended&&E(0);p.play()}else v(function(){t.play()})};t.pause=function(){r.paused=true;L?p.pause():v(function(){t.pause()})};Object.defineProperties(t,{src:{get:function(){return r.src},set:function(C){C&&C!==r.src&&c(C)}},autoplay:{get:function(){return r.autoplay},set:function(C){r.autoplay=t._util.isAttributeSet(C)}},loop:{get:function(){return r.loop},set:function(C){r.loop=t._util.isAttributeSet(C)}},width:{get:function(){return D.width},
...
0?r.muted:r.volume)/100},set:function(C){if(C<0||C>1)throw"Volume value must be between 0.0 and 1.0";C*=100;l(C)}},muted:{get:function(){return r.muted>0},set:function(C){j(t._util.isAttributeSet(C))}},error:{get:function(){return r.error}},controls:{get:function(){return r.controls},set:function(C){s(!!C)}}});t._canPlaySrc=m.HTMLSoundCloudAudioElement._canPlaySrc;t.canPlayType=m.HTMLSoundCloudAudioElement.canPlayType;return t}var h=16,e="",n=false,u=false,q=[];m.HTMLSoundCloudAudioElement=function(x){return new f(x)};
m.HTMLSoundCloudAudioElement._canPlaySrc=function(x){return/(?:https?:\/\/www\.|https?:\/\/|www\.|\.|^)(soundcloud)/.test(x)?"probably":e};m.HTMLSoundCloudAudioElement.canPlayType=function(x){return x==="audio/x-soundcloud"?"probably":e}})(Popcorn,window,document);(function(m,i,k){function d(n){var u=this,q=n.src.split("?")[0];if(q.substr(0,2)==="//")q=i.location.protocol+q;"play pause paused seekTo unload getCurrentTime getDuration getVideoEmbedCode getVideoHeight getVideoWidth getVideoUrl getColor setColor setLoop getVolume setVolume addEventListener".split(" ").forEach(function(x){u[x]=function(v){v=JSON.stringify({method:x,value:v});n.contentWindow&&n.contentWindow.postMessage(v,q)}})}function b(n){function u(w){y.unshift(w)}function q(w){var p=c.duration;
if(p!==w){c.duration=w;A.dispatchEvent("durationchange");if(isNaN(p)){c.networkState=A.NETWORK_IDLE;c.readyState=A.HAVE_METADATA;A.dispatchEvent("loadedmetadata");A.dispatchEvent("loadeddata");c.readyState=A.HAVE_FUTURE_DATA;A.dispatchEvent("canplay");c.readyState=A.HAVE_ENOUGH_DATA;A.dispatchEvent("canplaythrough");c.autoplay&&A.play();for(w=y.length;w--;){y[w]();delete y[w]}}}}function x(w){if(l){c.seeking=true;A.dispatchEvent("seeking");s.seekTo(w)}else u(function(){x(w)})}function v(){A.dispatchEvent("timeupdate")}
function o(w){(c.currentTime=w)!==L&&A.dispatchEvent("timeupdate");L=c.currentTime}function z(w){if(w.origin===e){var p;try{p=JSON.parse(w.data)}catch(B){console.warn(B)}if(p.player_id==j)switch(p.event){case "ready":s=new d(a);s.addEventListener("loadProgress");s.addEventListener("pause");s.setVolume(0);s.play();break;case "loadProgress":if(parseFloat(p.data.duration)>0&&!l){l=true;s.pause()}break;case "pause":s.setVolume(1);i.removeEventListener("message",z,false);i.addEventListener("message",I,
false);s.addEventListener("loadProgress");s.addEventListener("playProgress");s.addEventListener("play");s.addEventListener("pause");s.addEventListener("finish");s.addEventListener("seek");s.getDuration();c.networkState=A.NETWORK_LOADING;A.dispatchEvent("loadstart");A.dispatchEvent("progress")}}}function I(w){if(w.origin===e){var p;try{p=JSON.parse(w.data)}catch(B){console.warn(B)}if(p.player_id==j){switch(p.method){case "getCurrentTime":o(parseFloat(p.value));break;case "getDuration":q(parseFloat(p.value));
break;case "getVolume":w=parseFloat(p.value);if(c.volume!==w){c.volume=w;A.dispatchEvent("volumechange")}}switch(p.event){case "loadProgress":A.dispatchEvent("progress");q(parseFloat(p.data.duration));break;case "playProgress":o(parseFloat(p.data.seconds));break;case "play":c.ended&&x(0);if(!r){r=setInterval(E,f);c.loop&&A.dispatchEvent("play")}D=setInterval(v,A._util.TIMEUPDATE_MS);c.paused=false;if(t){t=false;c.loop||A.dispatchEvent("play");A.dispatchEvent("playing")}break;case "pause":c.paused=
true;if(!t){t=true;clearInterval(D);A.dispatchEvent("pause")}break;case "finish":if(c.loop){x(0);A.play()}else{c.ended=true;A.dispatchEvent("ended")}break;case "seek":o(parseFloat(p.data.seconds));c.seeking=false;A.dispatchEvent("timeupdate");A.dispatchEvent("seeked");A.dispatchEvent("canplay");A.dispatchEvent("canplaythrough")}}}}function E(){s.getCurrentTime()}function g(w){c.volume=w;if(l){s.setVolume(w);A.dispatchEvent("volumechange")}else u(function(){g(w)})}function F(w){if(l)if(w){c.muted=
c.volume;g(0)}else{c.muted=0;g(c.muted)}else{c.muted=w?1:0;u(function(){F(w)})}}if(!i.postMessage)throw"ERROR: HTMLVimeoVideoElement requires window.postMessage";var A=new m._MediaElementProto,O=typeof n==="string"?m.dom.find(n):n,a=k.createElement("iframe"),c={src:h,networkState:A.NETWORK_EMPTY,readyState:A.HAVE_NOTHING,seeking:false,autoplay:h,preload:h,controls:false,loop:false,poster:h,volume:1,muted:0,currentTime:0,duration:NaN,ended:false,paused:true,error:null},l=false,j=m.guid(),s,t=true,
y=[],D,r,L=0;A._eventNamespace=m.guid("HTMLVimeoVideoElement::");A.parentNode=O;A._util.type="Vimeo";A.play=function(){c.paused=false;l?s.play():u(function(){A.play()})};A.pause=function(){c.paused=true;l?s.pause():u(function(){A.pause()})};Object.defineProperties(A,{src:{get:function(){return c.src},set:function(w){if(w&&w!==c.src)if(A._canPlaySrc(w)){c.src=w;if(l)if(l&&s){clearInterval(r);s.pause();i.removeEventListener("message",I,false);O.removeChild(a);a=k.createElement("iframe")}l=false;w=A._util.parseUri(w);
var p=w.queryKey,B,Q=["api=1","player_id="+j,"title=0","byline=0","portrait=0"];c.loop=p.loop==="1"||c.loop;delete p.loop;c.autoplay=p.autoplay==="1"||c.autoplay;delete p.autoplay;w=e+"/video/"+/\d+$/.exec(w.path)+"?";for(B in p)p.hasOwnProperty(B)&&Q.push(encodeURIComponent(B)+"="+encodeURIComponent(p[B]));w+=Q.join("&");a.id=j;a.style.width="100%";a.style.height="100%";a.frameBorder=0;a.webkitAllowFullScreen=true;a.mozAllowFullScreen=true;a.allowFullScreen=true;O.appendChild(a);a.src=w;i.addEventListener("message",
z,false)}else{c.error={name:"MediaError",message:"Media Source Not Supported",code:MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED};A.dispatchEvent("error")}}},autoplay:{get:function(){return c.autoplay},set:function(w){c.autoplay=A._util.isAttributeSet(w)}},loop:{get:function(){return c.loop},set:function(w){c.loop=A._util.isAttributeSet(w)}},width:{get:function(){return A.parentNode.offsetWidth}},height:{get:function(){return A.parentNode.offsetHeight}},currentTime:{get:function(){return c.currentTime},
set:function(w){x(w)}},duration:{get:function(){return c.duration}},ended:{get:function(){return c.ended}},paused:{get:function(){return c.paused}},seeking:{get:function(){return c.seeking}},readyState:{get:function(){return c.readyState}},networkState:{get:function(){return c.networkState}},volume:{get:function(){return c.muted>0?c.muted:c.volume},set:function(w){if(w<0||w>1)throw"Volume value must be between 0.0 and 1.0";g(w)}},muted:{get:function(){return c.muted>0},set:function(w){F(A._util.isAttributeSet(w))}},
error:{get:function(){return c.error}}});A._canPlaySrc=m.HTMLVimeoVideoElement._canPlaySrc;A.canPlayType=m.HTMLVimeoVideoElement.canPlayType;return A}var f=16,h="",e="https://player.vimeo.com";m.HTMLVimeoVideoElement=function(n){return new b(n)};m.HTMLVimeoVideoElement._canPlaySrc=function(n){return/player.vimeo.com\/video\/\d+/.test(n)||/vimeo.com\/\d+/.test(n)?"probably":h};m.HTMLVimeoVideoElement.canPlayType=function(n){return n==="video/x-vimeo"?"probably":h}})(Popcorn,window,document);(function(m,i,k){function d(){var z;if(YT.loaded)for(x=true;o.length;){z=o.shift();z()}else setTimeout(d,250)}function b(){var z;if(!v){if(i.YT)d();else{z=k.createElement("script");z.addEventListener("load",d,false);z.src="https://www.youtube.com/iframe_api";k.head.appendChild(z)}v=true}return x}function f(z){o.push(z)}function h(z){function I(J){W.push(J)}function E(){R.pauseVideo();j("play",E);l("play",K)}function g(){l("pause",M);j("pause",g)}function F(){var J=function(){if(R.isMuted()){l("play",
...
MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED;break;default:N.message="Unknown error.";N.code=5}G.error=N;H.dispatchEvent("error")}function O(){l("play",K);l("pause",M);if(G.autoplay||!G.paused){j("play",O);G.paused=false;I(function(){G.paused||K()})}G.muted||R.unMute();G.readyState=H.HAVE_METADATA;H.dispatchEvent("loadedmetadata");aa=setInterval(r,e);H.dispatchEvent("loadeddata");G.readyState=H.HAVE_FUTURE_DATA;H.dispatchEvent("canplay");U=true;for(ba=setInterval(L,50);W.length;){W[0]();W.shift()}G.readyState=
H.HAVE_ENOUGH_DATA;H.dispatchEvent("canplaythrough")}function a(){j("pause",a);if(R.getCurrentTime()>0)setTimeout(a,0);else if(G.autoplay||!G.paused){l("play",O);R.playVideo()}else O()}function c(){j("play",c);if(R.getCurrentTime()===0)setTimeout(c,0);else{l("pause",a);R.seekTo(0);R.pauseVideo()}}function l(J,N){H.addEventListener("youtube-"+J,N,false)}function j(J,N){H.removeEventListener("youtube-"+J,N,false)}function s(J){H.dispatchEvent("youtube-"+J)}function t(){G.networkState=H.NETWORK_LOADING;
H.dispatchEvent("waiting")}function y(J){switch(J.data){case YT.PlayerState.ENDED:s("ended");break;case YT.PlayerState.PLAYING:s("play");break;case YT.PlayerState.PAUSED:R.getDuration()!==R.getCurrentTime()&&s("pause");break;case YT.PlayerState.BUFFERING:s("buffering")}J.data!==YT.PlayerState.BUFFERING&&ca===YT.PlayerState.BUFFERING&&H.dispatchEvent("progress");ca=J.data}function D(J){if(H._canPlaySrc(J)){G.src=J;if(b()){if(V)if(U){if(V&&R){j("buffering",t);j("ended",C);j("play",K);j("pause",M);M();
Z=U=false;G.currentTime=0;W=[];clearInterval(aa);clearInterval(ba);R.stopVideo();R.clearVideo();R.destroy();S=k.createElement("div")}}else{I(function(){D(J)});return}P.appendChild(S);var N=H._util.parseUri(J).queryKey;delete N.v;G.autoplay=N.autoplay==="1"||G.autoplay;delete N.autoplay;G.loop=N.loop==="1"||G.loop;delete N.loop;N.rel=N.rel||0;N.modestbranding=N.modestbranding||1;N.iv_load_policy=N.iv_load_policy||3;N.disablekb=N.disablekb||1;N.showinfo=N.showinfo||0;var ea=i.location.protocol==="file:"?
"*":i.location.protocol+"//"+i.location.host;N.origin=N.origin||ea;N.controls=N.controls||G.controls?2:0;G.controls=N.controls;N.wmode=N.wmode||"opaque";J=u.exec(J)[1];m.getJSONP("https://gdata.youtube.com/feeds/api/videos/"+J+"?v=2&alt=jsonc&callback=?",function(X){if(X.error)console.warn("failed to retreive duration data, reason: "+X.error.message);else if(X.data){G.duration=X.data.duration;H.dispatchEvent("durationchange");R=new YT.Player(S,{width:"100%",height:"100%",wmode:N.wmode,videoId:J,playerVars:N,
events:{onReady:F,onError:A,onStateChange:y}});G.networkState=H.NETWORK_LOADING;H.dispatchEvent("loadstart");H.dispatchEvent("progress")}else console.warn("failed to retreive duration data, reason: no response data")})}else f(function(){D(J)})}else{G.error={name:"MediaError",message:"Media Source Not Supported",code:MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED};H.dispatchEvent("error")}}function r(){var J=R.getCurrentTime();if(G.seeking)q(J-G.currentTime)<1&&Q();else{if(q(G.currentTime-J)>e){B();Q()}G.currentTime=
J}}function L(){var J=R.getVideoLoadedFraction();if(J&&$!==J){$=J;H.dispatchEvent("progress")}}function w(J){if(J!==G.currentTime){G.currentTime=J;if(U){B();R.seekTo(J)}else I(function(){B();R.seekTo(J)})}}function p(){H.dispatchEvent("timeupdate")}function B(){l("pause",g);j("pause",M);G.seeking=true;H.dispatchEvent("seeking")}function Q(){G.ended=false;G.seeking=false;H.dispatchEvent("timeupdate");H.dispatchEvent("seeked");H.dispatchEvent("canplay");H.dispatchEvent("canplaythrough")}function K(){if(G.ended){w(0);
G.ended=false}da=setInterval(p,H._util.TIMEUPDATE_MS);G.paused=false;if(Y){Y=false;if(G.loop&&!Z||!G.loop){Z=true;H.dispatchEvent("play")}H.dispatchEvent("playing")}}function M(){G.paused=true;if(!Y){Y=true;clearInterval(da);H.dispatchEvent("pause")}}function C(){if(G.loop){w(0);H.play()}else{G.ended=true;M();l("play",E);j("play",K);H.dispatchEvent("timeupdate");H.dispatchEvent("ended")}}function T(J){G.muted=J;if(U){R[J?"mute":"unMute"]();H.dispatchEvent("volumechange")}else I(function(){T(G.muted)})}
if(!i.postMessage)throw"ERROR: HTMLYouTubeVideoElement requires window.postMessage";var H=new m._MediaElementProto,P=typeof z==="string"?k.querySelector(z):z,S=k.createElement("div"),G={src:n,networkState:H.NETWORK_EMPTY,readyState:H.HAVE_NOTHING,seeking:false,autoplay:n,preload:n,controls:false,loop:false,poster:n,volume:1,muted:false,currentTime:0,duration:NaN,ended:false,paused:true,error:null},V=false,U=false,Z=false,R,Y=true,W=[],ca=-1,ba,$=0,aa,da;H._eventNamespace=m.guid("HTMLYouTubeVideoElement::");
H.parentNode=P;H._util.type="YouTube";l("buffering",t);l("ended",C);H.play=function(){G.paused=false;U?R.playVideo():I(function(){H.play()})};H.pause=function(){G.paused=true;if(U){g();R.pauseVideo()}else I(function(){H.pause()})};Object.defineProperties(H,{src:{get:function(){return G.src},set:function(J){J&&J!==G.src&&D(J)}},autoplay:{get:function(){return G.autoplay},set:function(J){G.autoplay=H._util.isAttributeSet(J)}},loop:{get:function(){return G.loop},set:function(J){G.loop=H._util.isAttributeSet(J)}},
width:{get:function(){return H.parentNode.offsetWidth}},height:{get:function(){return H.parentNode.offsetHeight}},currentTime:{get:function(){return G.currentTime},set:function(J){w(J)}},duration:{get:function(){return G.duration}},ended:{get:function(){return G.ended}},paused:{get:function(){return G.paused}},seeking:{get:function(){return G.seeking}},readyState:{get:function(){return G.readyState}},networkState:{get:function(){return G.networkState}},volume:{get:function(){return G.volume},set:function(J){if(J<
0||J>1)throw"Volume value must be between 0.0 and 1.0";G.volume=J;if(U){R.setVolume(G.volume*100);H.dispatchEvent("volumechange")}else I(function(){H.volume=J})}},muted:{get:function(){return G.muted},set:function(J){T(H._util.isAttributeSet(J))}},error:{get:function(){return G.error}},buffered:{get:function(){var J={start:function(N){if(N===0)return 0;throw"INDEX_SIZE_ERR: DOM Exception 1";},end:function(N){if(N===0){if(!G.duration)return 0;return G.duration*$}throw"INDEX_SIZE_ERR: DOM Exception 1";
}};Object.defineProperties(J,{length:{get:function(){return 1}}});return J},configurable:true}});H._canPlaySrc=m.HTMLYouTubeVideoElement._canPlaySrc;H.canPlayType=m.HTMLYouTubeVideoElement.canPlayType;return H}var e=10,n="",u=/^.*(?:\/|v=)(.{11})/,q=Math.abs,x=false,v=false,o=[];m.HTMLYouTubeVideoElement=function(z){return new h(z)};m.HTMLYouTubeVideoElement._canPlaySrc=function(z){return/(?:http:\/\/www\.|http:\/\/|www\.|\.|^)(youtu).*(?:\/|v=)(.{11})/.test(z)?"probably":n};m.HTMLYouTubeVideoElement.canPlayType=
function(z){return z==="video/x-youtube"?"probably":n}})(Popcorn,window,document);(function(m){var i=function(k,d){var b=0,f=0,h;m.forEach(d.classes,function(e,n){h=[];if(e==="parent")h[0]=document.querySelectorAll("#"+d.target)[0].parentNode;else h=document.querySelectorAll("#"+d.target+" "+e);b=0;for(f=h.length;b<f;b++)h[b].classList.toggle(n)})};m.compose("applyclass",{manifest:{about:{name:"Popcorn applyclass Effect",version:"0.1",author:"@scottdowne",website:"scottdowne.wordpress.com"},options:{}},_setup:function(k){k.classes={};k.applyclass=k.applyclass||"";for(var d=k.applyclass.replace(/\s/g,
...
preload:{elem:"input",type:"checkbox",label:"Preload","default":true},page:{elem:"input",type:"number",label:"Page Number",optional:true},aid:{elem:"input",type:"number",label:"Annotation Id",optional:true}}},_setup:function(d){function b(){function q(j){d._key=j.api.getId();d._changeView=function(s){d.aid?s.pageSet.showAnnotation(s.api.getAnnotation(d.aid)):s.api.setCurrentPage(d.page)}}function x(){k[d._key]={num:1,id:d._containerId};h.loaded=true}h.loaded=false;var v=d.url.replace(/\.html$/,".js"),
o=d.target,z=i.getElementById(o),I=i.createElement("div"),E=m.position(z),g=d.width||E.width;E=d.height||E.height;var F=d.sidebar||true,A=d.text||true,O=d.pdf||true,a=d.showAnnotations||true,c=d.zoom||700,l=d.search||true;if(!function(j){var s=false;m.forEach(h.viewers,function(t){if(t.api.getSchema().canonicalURL===j){q(t);t=k[d._key];d._containerId=t.id;t.num+=1;s=true;h.loaded=true}});return s}(d.url)){I.id=d._containerId=m.guid(o);o="#"+I.id;z.appendChild(I);e.trigger("documentready");h.load(v,
{width:g,height:E,sidebar:F,text:A,pdf:O,showAnnotations:a,zoom:c,search:l,container:o,afterLoad:d.page||d.aid?function(j){q(j);d._changeView(j);I.style.visibility="hidden";j.elements.pages.hide();x()}:function(j){q(j);x();I.style.visibility="hidden";j.elements.pages.hide()}})}}function f(){window.DV.loaded?b():setTimeout(f,25)}var h=window.DV=window.DV||{},e=this;if(h.loading)f();else{h.loading=true;h.recordHit="//www.documentcloud.org/pixel.gif";var n=i.createElement("link"),u=i.getElementsByTagName("head")[0];
...
b){var f=i.getElementById(b._containerId);if(f&&DV.viewers[b._key]){f.style.visibility="hidden";DV.viewers[b._key].elements.pages.hide()}},_teardown:function(d){var b=i.getElementById(d._containerId);if((d=d._key)&&DV.viewers[d]&&--k[d].num===0){for(DV.viewers[d].api.unload();b.hasChildNodes();)b.removeChild(b.lastChild);b.parentNode.removeChild(b)}}})})(Popcorn,window.document);(function(m){var i=0;m.plugin("flickr",function(k){var d,b=document.getElementById(k.target),f,h,e,n,u=k.numberofimages||4,q=k.height||"50px",x=k.width||"50px",v=k.padding||"5px",o=k.border||"0px";d=document.createElement("div");d.id="flickr"+i;d.style.width="100%";d.style.height="100%";d.style.display="none";i++;b&&b.appendChild(d);var z=function(){if(f)setTimeout(function(){z()},5);else{h="http://api.flickr.com/services/rest/?method=flickr.people.findByUsername&";h+="username="+k.username+"&api_key="+
k.apikey+"&format=json&jsoncallback=flickr";m.getJSONP(h,function(E){f=E.user.nsid;I()})}},I=function(){h="http://api.flickr.com/services/feeds/photos_public.gne?";if(f)h+="id="+f+"&";if(k.tags)h+="tags="+k.tags+"&";h+="lang=en-us&format=json&jsoncallback=flickr";m.xhr.getJSONP(h,function(E){var g=document.createElement("div");g.innerHTML="<p style='padding:"+v+";'>"+E.title+"<p/>";m.forEach(E.items,function(F,A){if(A<u){e=document.createElement("a");e.setAttribute("href",F.link);e.setAttribute("target",
"_blank");n=document.createElement("img");n.setAttribute("src",F.media.m);n.setAttribute("height",q);n.setAttribute("width",x);n.setAttribute("style","border:"+o+";padding:"+v);e.appendChild(n);g.appendChild(e)}else return false});d.appendChild(g)})};if(k.username&&k.apikey)z();else{f=k.userid;I()}k.toString=function(){return k.tags||k.username||"Flickr"};return{start:function(){d.style.display="inline"},end:function(){d.style.display="none"},_teardown:function(E){document.getElementById(E.target)&&
...
start:function(i,k){k._container.style.display="inline"},end:function(i,k){k._container.style.display="none"},_teardown:function(i){var k=m.dom.find(i.target);k&&k.removeChild(i._container)}})})(Popcorn);(function(m){var i=1,k=false;m.plugin("googlefeed",function(d){var b=function(){var n=false,u=0,q=document.getElementsByTagName("link"),x=q.length,v=document.head||document.getElementsByTagName("head")[0],o=document.createElement("link");if(window.GFdynamicFeedControl)k=true;else m.getScript("//www.google.com/uds/solutions/dynamicfeed/gfdynamicfeedcontrol.js",function(){k=true});for(;u<x;u++)if(q[u].href==="//www.google.com/uds/solutions/dynamicfeed/gfdynamicfeedcontrol.css")n=true;if(!n){o.type=
"text/css";o.rel="stylesheet";o.href="//www.google.com/uds/solutions/dynamicfeed/gfdynamicfeedcontrol.css";v.insertBefore(o,v.firstChild)}};window.google?b():m.getScript("//www.google.com/jsapi",function(){google.load("feeds","1",{callback:function(){b()}})});var f=document.createElement("div"),h=document.getElementById(d.target),e=function(){if(k)d.feed=new GFdynamicFeedControl(d.url,f,{vertical:d.orientation.toLowerCase()==="vertical"?true:false,horizontal:d.orientation.toLowerCase()==="horizontal"?
...
google.maps.LatLng){f=new google.maps.Geocoder;m.getScript("//maps.stamen.com/js/tile.stamen.js",function(){b=true})}else setTimeout(function(){googleCallback(e)},1)};h=function(){if(document.body){d=true;m.getScript("//maps.google.com/maps/api/js?sensor=false&callback=googleCallback")}else setTimeout(function(){h()},1)};m.plugin("googlemap",function(e){var n,u,q,x=document.getElementById(e.target);e.type=e.type||"ROADMAP";e.zoom=e.zoom||1;e.lat=e.lat||0;e.lng=e.lng||0;d||h();n=document.createElement("div");
n.id="actualmap"+k;n.style.width=e.width||"100%";n.style.height=e.height?e.height:x&&x.clientHeight?x.clientHeight+"px":"100%";k++;x&&x.appendChild(n);var v=function(){if(b){if(n)if(e.location)f.geocode({address:e.location},function(o,z){if(n&&z===google.maps.GeocoderStatus.OK){e.lat=o[0].geometry.location.lat();e.lng=o[0].geometry.location.lng();q=new google.maps.LatLng(e.lat,e.lng);u=i(e,q,n)}});else{q=new google.maps.LatLng(e.lat,e.lng);u=i(e,q,n)}}else setTimeout(function(){v()},5)};v();e.toString=
function(){return e.location||(e.lat&&e.lng?e.lat+", "+e.lng:e._natives.manifest.options.location["default"])};return{start:function(o,z){var I=this,E,g=function(){if(u){z._map=u;u.getDiv().style.display="block";google.maps.event.trigger(u,"resize");u.setCenter(q);if(z.zoom&&typeof z.zoom!=="number")z.zoom=+z.zoom;u.setZoom(z.zoom);if(z.heading&&typeof z.heading!=="number")z.heading=+z.heading;if(z.pitch&&typeof z.pitch!=="number")z.pitch=+z.pitch;if(z.type==="STREETVIEW"){u.setStreetView(E=new google.maps.StreetViewPanorama(n,
{position:q,pov:{heading:z.heading=z.heading||0,pitch:z.pitch=z.pitch||0,zoom:z.zoom}}));var F=function(s,t){var y=google.maps.geometry.spherical.computeHeading;setTimeout(function(){var D=I.media.currentTime;if(typeof z.tween==="object"){for(var r=0,L=s.length;r<L;r++){var w=s[r];if(D>=w.interval*(r+1)/1E3&&(D<=w.interval*(r+2)/1E3||D>=w.interval*L/1E3)){l.setPosition(new google.maps.LatLng(w.position.lat,w.position.lng));l.setPov({heading:w.pov.heading||y(w,s[r+1])||0,zoom:w.pov.zoom||0,pitch:w.pov.pitch||
0})}}F(s,s[0].interval)}else{r=0;for(L=s.length;r<L;r++){w=z.interval;if(D>=w*(r+1)/1E3&&(D<=w*(r+2)/1E3||D>=w*L/1E3)){A.setPov({heading:y(s[r],s[r+1])||0,zoom:z.zoom,pitch:z.pitch||0});A.setPosition(O[r])}}F(O,z.interval)}},t)};if(z.location&&typeof z.tween==="string"){var A=E,O=[],a=new google.maps.DirectionsService,c=new google.maps.DirectionsRenderer(A);a.route({origin:z.location,destination:z.tween,travelMode:google.maps.TravelMode.DRIVING},function(s,t){if(t==google.maps.DirectionsStatus.OK){c.setDirections(s);
for(var y=s.routes[0].overview_path,D=0,r=y.length;D<r;D++)O.push(new google.maps.LatLng(y[D].lat(),y[D].lng()));z.interval=z.interval||1E3;F(O,10)}})}else if(typeof z.tween==="object"){var l=E;a=0;for(var j=z.tween.length;a<j;a++){z.tween[a].interval=z.tween[a].interval||1E3;F(z.tween,10)}}}z.onmaploaded&&z.onmaploaded(z,u)}else setTimeout(function(){g()},13)};g()},end:function(){if(u)u.getDiv().style.display="none"},_teardown:function(o){var z=document.getElementById(o.target);z&&z.removeChild(n);
...
"none";f._container.style.width="0px";f._container.style.height="0px";f._container.style.visibility="hidden";f._container.style.overflow="hidden";f.popcorn.pause()},_teardown:function(b){b.popcorn&&b.popcorn.destory&&b.popcorn.destroy();document.getElementById(b.target)&&document.getElementById(b.target).removeChild(b._container)}})})(Popcorn,this);(function(m){m.plugin("mustache",function(i){var k,d,b,f;m.getScript("http://mustache.github.com/extras/mustache.js");var h=!!i.dynamic,e=typeof i.template,n=typeof i.data,u=document.getElementById(i.target);i.container=u||document.createElement("div");if(e==="function")if(h)b=i.template;else f=i.template(i);else f=e==="string"?i.template:"";if(n==="function")if(h)k=i.data;else d=i.data(i);else d=n==="string"?JSON.parse(i.data):n==="object"?i.data:"";return{start:function(q,x){var v=function(){if(window.Mustache){if(k)d=
k(x);if(b)f=b(x);var o=Mustache.to_html(f,d).replace(/^\s*/mg,"");x.container.innerHTML=o}else setTimeout(function(){v()},10)};v()},end:function(q,x){x.container.innerHTML=""},_teardown:function(){k=d=b=f=null}}},{about:{name:"Popcorn Mustache Plugin",version:"0.1",author:"David Humphrey (@humphd)",website:"http://vocamus.net/dave"},options:{start:{elem:"input",type:"number",label:"Start"},end:{elem:"input",type:"number",label:"End"},target:"mustache-container",template:{elem:"input",type:"text",
label:"Template"},data:{elem:"input",type:"text",label:"Data"},dynamic:{elem:"input",type:"checkbox",label:"Dynamic","default":true}}})})(Popcorn);(function(m){function i(d,b){if(d.map)d.map.div.style.display=b;else setTimeout(function(){i(d,b)},10)}var k=1;m.plugin("openmap",function(d){var b,f,h,e,n,u,q,x,v=document.getElementById(d.target);b=document.createElement("div");b.id="openmapdiv"+k;b.style.width="100%";b.style.height="100%";k++;v&&v.appendChild(b);x=function(){if(window.OpenLayers&&window.OpenLayers.Layer.Stamen){if(d.location){location=new OpenLayers.LonLat(0,0);m.getJSONP("//tinygeocoder.com/create-api.php?q="+d.location+"&callback=jsonp",
...
delete i.target}}})})(Popcorn);(function(m){var i={},k=0,d=document.createElement("span"),b=["webkit","Moz","ms","O",""],f=["Transform","TransitionDuration","TransitionTimingFunction"],h={},e;document.getElementsByTagName("head")[0].appendChild(d);for(var n=0,u=f.length;n<u;n++)for(var q=0,x=b.length;q<x;q++){e=b[q]+f[n];if(e in d.style){h[f[n].toLowerCase()]=e;break}}document.getElementsByTagName("head")[0].appendChild(d);m.plugin("wordriver",{manifest:{about:{name:"Popcorn WordRiver Plugin"},options:{start:{elem:"input",type:"number",
label:"Start"},end:{elem:"input",type:"number",label:"End"},target:"wordriver-container",text:{elem:"input",type:"text",label:"Text","default":"Popcorn.js"},color:{elem:"input",type:"text",label:"Color","default":"Green",optional:true}}},_setup:function(v){v._duration=v.end-v.start;var o;if(!(o=i[v.target])){o=v.target;i[o]=document.createElement("div");var z=document.getElementById(o);z&&z.appendChild(i[o]);i[o].style.height="100%";i[o].style.position="relative";o=i[o]}v._container=o;v.word=document.createElement("span");
v.word.style.position="absolute";v.word.style.whiteSpace="nowrap";v.word.style.opacity=0;v.word.style.MozTransitionProperty="opacity, -moz-transform";v.word.style.webkitTransitionProperty="opacity, -webkit-transform";v.word.style.OTransitionProperty="opacity, -o-transform";v.word.style.transitionProperty="opacity, transform";v.word.style[h.transitionduration]="1s, "+v._duration+"s";v.word.style[h.transitiontimingfunction]="linear";v.word.innerHTML=v.text;v.word.style.color=v.color||"black"},start:function(v,
o){o._container.appendChild(o.word);o.word.style[h.transform]="";o.word.style.fontSize=~~(30+20*Math.random())+"px";k%=o._container.offsetWidth-o.word.offsetWidth;o.word.style.left=k+"px";k+=o.word.offsetWidth+10;o.word.style[h.transform]="translateY("+(o._container.offsetHeight-o.word.offsetHeight)+"px)";o.word.style.opacity=1;setTimeout(function(){o.word.style.opacity=0},(o.end-o.start-1||1)*1E3)},end:function(v,o){o.word.style.opacity=0},_teardown:function(v){var o=document.getElementById(v.target);
v.word.parentNode&&v._container.removeChild(v.word);i[v.target]&&!i[v.target].childElementCount&&o&&o.removeChild(i[v.target])&&delete i[v.target]}})})(Popcorn);(function(m){m.parser("parseJSON","JSON",function(i){var k={title:"",remote:"",data:[]};m.forEach(i.data,function(d){k.data.push(d)});return k})})(Popcorn);(function(m){m.parser("parseSBV",function(i){var k={title:"",remote:"",data:[]},d=[],b=0,f=0,h=function(v){v=v.split(":");var o=v.length-1,z;try{z=parseInt(v[o-1],10)*60+parseFloat(v[o],10);if(o===2)z+=parseInt(v[0],10)*3600}catch(I){throw"Bad cue";}return z},e=function(v,o){var z={};z[v]=o;return z};i=i.text.split(/(?:\r\n|\r|\n)/gm);for(f=i.length;b<f;){var n={},u=[],q=i[b++].split(",");try{n.start=h(q[0]);for(n.end=h(q[1]);b<f&&i[b];)u.push(i[b++]);n.text=u.join("<br />");d.push(e("subtitle",n))}catch(x){for(;b<
f&&i[b];)b++}for(;b<f&&!i[b];)b++}k.data=d;return k})})(Popcorn);(function(m){function i(d,b){var f={};f[d]=b;return f}function k(d){d=d.split(":");try{var b=d[2].split(",");if(b.length===1)b=d[2].split(".");return parseFloat(d[0],10)*3600+parseFloat(d[1],10)*60+parseFloat(b[0],10)+parseFloat(b[1],10)/1E3}catch(f){return 0}}m.parser("parseSRT",function(d,b){var f={title:"",remote:"",data:[]},h=[],e=0,n=0,u,q,x,v,o;u=d.text.split(/(?:\r\n|\r|\n)/gm);for(x=u.length-1;x>=0&&!u[x];)x--;v=x+1;for(e=0;e<v;e++){o={};x=[];for(e=e;!u[e];)e++;e=e;o.id=parseInt(u[e++],10);
q=u[e++].split(/[\t ]*--\>[\t ]*/);o.start=k(q[0]);n=q[1].indexOf(" ");if(n!==-1)q[1]=q[1].substr(0,n);for(o.end=k(q[1]);e<v&&u[e];)x.push(u[e++]);o.text=x.join("\\N").replace(/\{(\\[\w]+\(?([\w\d]+,?)+\)?)+\}/gi,"");o.text=o.text.replace(/</g,"&lt;").replace(/>/g,"&gt;");o.text=o.text.replace(/&lt;(\/?(font|b|u|i|s))((\s+(\w|\w[\w\-]*\w)(\s*=\s*(?:\".*?\"|'.*?'|[^'\">\s]+))?)+\s*|\s*)(\/?)&gt;/gi,"<$1$3$7>");o.text=o.text.replace(/\\N/gi,"<br />");if(b&&b.target)o.target=b.target;h.push(i("subtitle",
o))}f.data=h;return f})})(Popcorn);(function(m){function i(b,f){var h=b.substr(10).split(","),e;e={start:k(h[f.start]),end:k(h[f.end])};if(e.start===-1||e.end===-1)throw"Invalid time";var n=v.call(q,/\{(\\[\w]+\(?([\w\d]+,?)+\)?)+\}/gi,""),u=n.replace,q;q=h.length;v=[];for(var x=f.text;x<q;x++)v.push(h[x]);q=v.join(",");var v=q.replace;e.text=u.call(n,/\\N/gi,"<br />");return e}function k(b){var f=b.split(":");if(b.length!==10||f.length<3)return-1;return parseInt(f[0],10)*3600+parseInt(f[1],10)*60+parseFloat(f[2],10)}function d(b,
f){var h={};h[b]=f;return h}m.parser("parseSSA",function(b){var f={title:"",remote:"",data:[]},h=[],e=0,n;b=b.text.split(/(?:\r\n|\r|\n)/gm);for(n=b.length;e<n&&b[e]!=="[Events]";)e++;var u=b[++e].substr(8).split(", "),q={},x,v;v=0;for(x=u.length;v<x;v++)if(u[v]==="Start")q.start=v;else if(u[v]==="End")q.end=v;else if(u[v]==="Text")q.text=v;for(;++e<n&&b[e]&&b[e][0]!=="[";)try{h.push(d("subtitle",i(b[e],q)))}catch(o){}f.data=h;return f})})(Popcorn);(function(m){function i(e,n,u){var q=e.firstChild;e=k(e,u);u=[];for(var x;q;){if(q.nodeType===1)if(q.nodeName==="p")u.push(d(q,n,e));else if(q.nodeName==="div"){x=b(q.getAttribute("begin"));if(x<0)x=n;u.push.apply(u,i(q,x,e))}q=q.nextSibling}return u}function k(e,n){var u=e.getAttribute("region");return u!==null?u:n||""}function d(e,n,u){var q={};q.text=(e.textContent||e.text).replace(f,"").replace(h,"<br />");q.id=e.getAttribute("xml:id")||e.getAttribute("id");q.start=b(e.getAttribute("begin"),n);
q.end=b(e.getAttribute("end"),n);q.target=k(e,u);if(q.end<0){q.end=b(e.getAttribute("duration"),0);if(q.end>=0)q.end+=q.start;else q.end=Number.MAX_VALUE}return{subtitle:q}}function b(e,n){var u;if(!e)return-1;try{return m.util.toSeconds(e)}catch(q){for(var x=e.length-1;x>=0&&e[x]<="9"&&e[x]>="0";)x--;u=x;x=parseFloat(e.substring(0,u));u=e.substring(u);return x*({h:3600,m:60,s:1,ms:0.0010}[u]||-1)+(n||0)}}var f=/^[\s]+|[\s]+$/gm,h=/(?:\r\n|\r|\n)/gm;m.parser("parseTTML",function(e){var n={title:"",
